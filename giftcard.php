...
class giftcard extends Module
{
    ...
    public function installDb()
    {
        if (!Db::getInstance()->execute('
            ...
            `display_currency` TINYINT( 1 ) UNSIGNED NOT NULL DEFAULT 0,
            `display_two_decimals` TINYINT( 1 ) UNSIGNED NOT NULL DEFAULT 0,
    ...
    private function deleteProductGiftCard($id)
    {
        ...
        if ($awaiting_order = $this->checkAwaitingOrder($id_product)) {
            return $this->_html .= $this->displayError(
                $this->l('This product could not be deleted, an associated order is awaiting payment').
                ': <a href="index.php?controller=AdminOrders&id_order='.
                $awaiting_order.
                '&vieworder&token='.
    ...
    public function cardExist($name)
    {
        $id_card = Db::getInstance()->getValue('
            ...
            LEFT JOIN `'._DB_PREFIX_.'product_shop` ps ON (pl.`id_product` = ps.`id_product`)
            WHERE pl.`name` = "'.$name.'"
    ...
    private function createProductGiftCard(
        $name,
        $value,
        $image_name,
        $display_duration,
        ...
        if ($this->cardExist($name)) {
            ...
            if ($id_category) {
                ...
                foreach (Language::getLanguages(true) as $lang) {
                    ...
                    $category->link_rewrite[$lang['id_lang']] = Tools::link_rewrite(
                        $this->lgc('Gift-Card', $lang['id_lang'])
                    );
                }
            ...
            foreach (Language::getLanguages(false) as $lang) {
                ...
                $product->link_rewrite[$lang['id_lang']] = $link_rewrite;
                 $partial = ($partial_use == 0) ?
                    $this->lgc('This voucher must be used in one time', $lang['id_lang']) :
                    $this->lgc(
                        'This voucher can be used several times (as long as the balance is positive)',
                        $lang['id_lang']
                    );
                 $usetax = $tax == 0 ? $this->l('WT', $lang['id_lang']) : $this->l('ATI', $lang['id_lang']);
                if ($display_duration) {
                    $desc_validity = $this->lgc('Date of mailing of the card by Email', $lang['id_lang']).
                        ' +'.
                        $duration.
                        ' '.
                        $this->lgc('Month', $lang['id_lang']);
                } else {
                    $desc_validity = Tools::displayDate($validity);
                }
                $product->description_short[$lang['id_lang']] = $partial.
                    '.<br/>'.
                    $this->lgc('The amount value will be use only in', $lang['id_lang']).
                    ' '.
                    $usetax.
                    '.<br/>'.
                    $this->lgc('Available until', $lang['id_lang']).
                    ': '.
                    $desc_validity;
            }
            ...
            Db::getInstance()->insert('giftcard', array(
                ...
                'display_two_decimals' => $display_two_decimals,
    ...
    public function generateImage(
        ...
        $price = $value;
        if ($display_two_decimals && $display_currency) {
            $price = Tools::displayPrice($price);
        } else if ($display_two_decimals && !$display_currency) {
            $price = number_format($price, 2, ',', ' ');
        } else if (!$display_two_decimals && $display_currency) {
            $price = Tools::ps_round($price, 2).' '.$this->context->currency->sign;
        }
        ...
        foreach ($imagesTypes as $k => $image_type) {
            if (!ImageManager::resize(
                $tmpName,
                $new_path.'-'.stripslashes($image_type['name']).'.'.$image->image_format,
                $image_type['width'],
                $image_type['height'],
                $image->image_format
            )) {
                return $this->errors[] = Tools::displayError('An error occurred while copying this image:').' '.
    ...
    private function addCustom($product)
    {
        $names = array(
            'Add a message on the card',
            'If you want send this card to a friend enter his Email address (If the email address is bad you will receive the card on your email).',
            'Friend first name (display in the mail)',
            'Friend last name (display in the mail)'
        );
    	$product->customizable = 1;
        $product->text_fields = (int)count($names);
        $product->createLabels(0, (int)count($names));
        if ($product->save()) {
            $id_customization_fields = Db::getInstance()->executeS('
            ...
            foreach (Language::getLanguages(false) as $lang) {
                foreach ($id_customization_fields as $key => $field) {
                    $labels = $this->lgc($names[$key], $lang['id_lang']);
                    Db::getInstance()->execute('
                        INSERT INTO `'._DB_PREFIX_.'customization_field_lang`
                        (`id_customization_field`, `id_lang`, `name`)
                        VALUES (
                            '.(int)$field['id_customization_field'].',
                            '.(int)$lang['id_lang'].',
                            "'.pSQL($labels).'"
                        )
                        ON DUPLICATE KEY UPDATE `name` = "'.pSQL($labels).'"'
                    );
                }
            }
        }
    ...
    * return x,y position in an array */
    private function getOrigin($font, $value, $size, $position_x, $position_y)
    {
        ...
        switch ($position_x) {
            case 0:
                ...
                break;
            case 1:
                $origin['x'] = (1000 - $width_text)/2;
                break;
            case 2:
                $origin['x'] = (950 - $width_text);
                break;
        }
        switch ($position_y) {
            case 0:
                $origin['y'] = 50 + $height_text;
                break;
            case 1:
                $origin['y'] = (600 + $height_text)/2;
                break;
            case 2:
                $origin['y'] = (590 - $box[1]);
                break;
    ...
    private function initOptionCreate($id_giftcard)
    {
            ...
            'form' => array(
                ...
                'input' => array(
                    ...
                    array(
                        ...
                        'desc' => $this->l('Only png format file accepted').' (1000 x 600 px)',
                        'name' => 'cardimage',
                        'hint' =>  $this->l('Upload your own image image.').
                            ' '.
                            $this->l('Only png format file accepted').
                            ' (1000 x 600 px)',
                        'class' => 'fixed-width-xxl',
                    ),
                    array(
                    ...
                    array(
                        'type' => 'switch',
                        'is_bool' => true,
                        'label' => $this->l('Display Two decimals'),
                        'name' => 'display_two_decimals',
                        'class' => 't',
                        'desc' => $this->l('Display Two decimals after point ex:(10.00)'),
                        'hint' => $this->l('Display Two decimals after point ex:(10.00)'),
                        'values' => array(
                            array(
                                'id' => 'active_on',
                                'value' => 1,
                    ...
                    array(
                        'type' => 'switch',
                        'is_bool' => true,
                        'label' => $this->l('Display Currency'),
                        'name' => 'display_currency',
                        'class' => 't',
                        'desc' => $this->l('Display Currency sign'),
                        'hint' => $this->l('Display Currency sign'),
                        'values' => array(
                            array(
                                'id' => 'active_on',
                                'value' => 1,
                    ...
                    array(
                        'type' => 'switch',
                        'is_bool' => true,
                        'label' => $this->l('Gift card use tax included'),
                        'name' => 'tax',
                        'class' => 't',
                    ...
                    array(
                        ...
                        'name' => 'free_shipping',
                        'class' => 't',
                        'desc' => $this->l('Enable free shipping for this gift card'),
                        'hint' => $this->l('Enable free shipping for this gift card'),
                        'values' => array(
                            array(
                                'id' => 'active_on',
                                'value' => 1,
                ...
                'submit' => array(
                    'title' => Tools::isSubmit('updategiftcard') ?
                        $this->l('Update Gift Card') :
                        $this->l('Create Product Gift Card'),
        ...
        if ($id_giftcard) {
            ...
            $helper->fields_value['display_currency'] = 0;
            $helper->fields_value['display_two_decimals'] = 0;
            ...
            $helper->fields_value['free_shipping'] = 0;
    ...
    public function getGiftcardFieldValues($id_giftcard)
    {
        ...
        $getprice = ($giftcard->tax == 1) ?
            $product['price'] + ($product['price'] * $this->getTaxRate() /100) :
            $product['price'];
        ...
        $fields_value = array(
            ...
            'values' => (float)$price,
            'tax' => $giftcard->tax,
            'display_currency' => $giftcard->display_currency,
            'display_two_decimals' => $giftcard->display_two_decimals,
            ...
            'free_shipping' => $giftcard->free_shipping,
    ...
    protected function giftcardList()
    {
        ...
        if ($this->_error) {
            ...
            $displayError = $this->displayError(
                '<ul>'.
                $this->l('An error at least occured. Unable to create this gift card: ').
                $errors.
                '</ul>'
            );
        }
    ...
    protected function getListContent($id_lang)
    {
        ...
        foreach ($ids_card as $id_card) {
            ...
            foreach ($folder as $file) {
                if ($file->isFile() &&
                    !$file->isDot() &&
                    (time() - $file->getMTime() > strtotime($id_card['validity']))
                ) {
            ...
            $price = ($id_card['tax'] == 1) ?
                $product->price + ($product->price * $this->getTaxRate()) /100 :
                $product->price;
            if ($id_card['display_two_decimals'] && $id_card['display_currency']) {
                $price = Tools::displayPrice($price);
            } else if ($id_card['display_two_decimals'] && !$id_card['display_currency']) {
                $price = number_format($price, 2, ',', ' ');
            } else if (!$id_card['display_two_decimals'] && $id_card['display_currency']) {
                 $price = Tools::ps_round($price, 2).' '.$currency->sign;
            } else if (!$id_card['display_two_decimals'] && !$id_card['display_currency']) {
                 $price = Tools::ps_round($price, 2);
            }
            if (empty($id_card['display_shadow']) ||
                $id_card['display_shadow'] = 0 ||
                ($id_card['colortext'] == $id_card['colorshadow'])
            ) {
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('newgiftCard') || Tools::isSubmit('updategiftcard')) {
            ...
            $this->_html .= '
                ...
                #drop-pos_code, #drop-pos_text {
                    width: 300px;
                    height: 180px;
                    background: url("'.$this->_path.'img/models/'.$model.'");
                    background-size: cover;
                    color:#fff;
                    margin-bottom: 10px;
                }
                .ui-widget-content {border: 1px solid #aaaaaa; color: #222222; float: left; border-radius: 5px;}
                .ui-widget-content a {color: #00aff0;text-decoration: none;}
                .ui-widget-content a:hover {color: #0077a3; text-decoration: underline;}
                </style>';
            return $this->_html.
                $this->initOptionCreate($id_giftcard).
                $space.
                $this->LicenseGratuite().
                $space.
                $this->giftcardList();
        } else {
            return $this->_html.$this->giftcardList().$space.$this->LicenseGratuite().$space.$this->_previewMail();
        }
    }

    public function LicenseGratuite()
    {
        $html ='
            <style>
                .ui-widget-content a {color: #00aff0;text-decoration: none;}
            ...
            <div class="panel">
                ...
                <p>'.$this->l('This module was created by').' <a href="http://loulou66.fr">Loulou66.fr</a>.</p>
                <p>'.$this->l('If you want make a donation for this module click on buton below.').'</p>
                <form action="https://www.paypal.com/cgi-bin/webscr" method="post" target="_top">
                    <input type="hidden" name="cmd" value="_s-xclick">
                    <input type="hidden" name="hosted_button_id" value="JJKDMM57RDCLL">
    ...
    public function postProcess() {
        ...
        if (Tools::isSubmit('submitCreateProduct')) {
            ...
            $display_currency = Tools::getValue('display_currency');
            $display_two_decimals = Tools::getValue('display_two_decimals');
            $partial_use = Tools::getValue('partial_use');
            $free_shipping = Tools::getValue('free_shipping');
            $highlight = Tools::getValue('highlight');
            if ((int)Configuration::get('PS_TAX') === 0 && $tax == 1) {
                return $this->_html .= $this->displayError(
                    $this->l('You can not create a card with tax if taxes are not enabled on your shop')
                );
            }
            foreach ($values as $value) {
                if (!$this->createProductGiftCard(
                    $name.'-'.$value,
                    (float)$value,
                    $image_name,
                    $display_duration,
            ...
            return $this->_html .= $this->displayConfirmation(
                $this->l('The Gift Card has been created  successfully.')
            );
        }
        ...
        if (Tools::isSubmit('submitUpdateProduct')) {
            ...
            if ($awaiting_order = $this->checkAwaitingOrder($id_product)) {
                return $this->_html .= $this->displayError(
                    $this->l('This product could not be modified, an associated order is awaiting payment').
                    ': <a href="index.php?controller=AdminOrders&id_order='.
                    $awaiting_order.
                    '&vieworder&token='.
                    Tools::getAdminTokenLite('AdminOrders').
            ...
            $giftcard->display_currency = Tools::getValue('display_currency');
            $giftcard->display_two_decimals = Tools::getValue('display_two_decimals');
            $giftcard->partial_use = Tools::getValue('partial_use');
            $giftcard->free_shipping = Tools::getValue('free_shipping');
            $giftcard->highlight = Tools::getValue('highlight');
            $giftcard->update();
            $product = new Product($id_product);
            foreach (Language::getLanguages(false) as $lang) {
                $product->name[$lang['id_lang']] = Tools::getValue('product_name');;
                $partial = ($giftcard->partial_use == 0) ?
                    $this->lgc('This voucher must be used in one time', $lang['id_lang']) :
                    $this->lgc(
                        'This voucher can be used several times (as long as the balance is positive)',
                        $lang['id_lang']
                    );
                 $usetax = ($giftcard->tax == 0) ?
                    $this->lgc('WT', $lang['id_lang']) :
                    $this->lgc('ATI', $lang['id_lang']);
                if ($giftcard->display_duration) {
                    $desc_validity =  $this->lgc('Date of mailing of the card by Email', $lang['id_lang']).
                    ' +'.$giftcard->duration.' '.$this->lgc('Month', $lang['id_lang']);
                } else {
                    $desc_validity = Tools::displayDate($validity);
                }
                $product->description_short[$lang['id_lang']] = $partial.
            ...
            $this->generateImage(
                ...
                $image_path,
                $new_price,
                $giftcard->display_currency,
                $giftcard->display_two_decimals,
                $giftcard->display_code,
                $giftcard->pos_code_x,
                $giftcard->pos_code_y,
                $giftcard->colorcode,
                $giftcard->display_text,
                $giftcard->pos_text_x,
                $giftcard->pos_text_y,
                $giftcard->text_size,
                $giftcard->colortext,
                $giftcard->display_shadow,
                $giftcard->colorshadow
            );
            ImageManager::thumbnail(
                $image_path,
                'product_mini_'.(int) $id_product.'_'.$this->context->shop->id.'.jpg',
                45,
                'jpg',
                true,
                false
        ...
        if (Tools::isSubmit('deletegiftcard')) {
            $id_giftcard = Tools::getValue('id_giftcard');
            return $this->deleteProductGiftCard($id_giftcard);
        }
    ...
    private function _previewMail()
    {
        ...
        $productLink = '#';
        $GiftCardDatas = '<p>'.
            $this->l('Your customized message.').
            '</p><br /><br /><p><span style="font-weight:bold;">'
            .$this->l('See the conditions of use on the product page').
            '</span><a href="'.$productLink.'"> '.
            $GiftCardName.
            '</a></p><br /><br />';
    ...
    private function createCard($id_order, $id_cart, $id_customer, $id_currency)
    {
        ...
        foreach ($order->getProducts() as $product) {
            if (in_array($product['product_id'], $card_products)) {
                ...
                $text_size = $giftcard->text_size;
                $colortext = $giftcard->colortext;
                $GiftCardConditions = ($giftcard->partial_use == 0) ?
                    $this->lgc('This voucher must be used in one time', $customer->id_lang) :
                    $this->lgc(
                        'This voucher can be used several times (as long as the balance is positive)',
                        $customer->id_lang
                    );
                $tax = $giftcard->tax == 0 ?
                    ' ('.$this->lgc('WT', $customer->id_lang).')' :
                    ' ('.$this->lgc('ATI', $customer->id_lang).')';
                    ...
                    foreach($product['customizedDatas'] as $customizationPerAddress) {
                        foreach ($customizationPerAddress as $customizationId => $customization) {
                            for ($i = 0; $i < $customization['quantity']; $i++) {
                                ...
                                $textcolor = $this->hex2rgb($colortext);
                                $shadowcolor = $this->hex2rgb($colorshadow);
                                $colorcode2 = imagecolorallocate(
                                    $imageView,
                                    $codecolor[0],
                                    $codecolor[1],
                                    $codecolor[2]
                                );
                                $colortext2 = imagecolorallocate(
                                    $imageView,
                                    $textcolor[0],
                                    $textcolor[1],
                                    $textcolor[2]
                                );
                                $colorshadow2 = imagecolorallocate(
                                    $imageView,
                                    $shadowcolor[0],
                                    $shadowcolor[1],
                                    $shadowcolor[2]
                                ...
                                // si le champs l'e-mail n'a pas été rempli on prend celle du client par default
                                $message = '';
                                $firstname = '';
                                $lastname = '';
                                $email = $customer->email;
                                // remplissage des champs et vérification pour l'e-mail ami meme si tous les chmaps ne sont pas remplis si l'e-mail non valide envoi de la carte sur l'e-mail du client avec un e-mail d'avertissement.
                                foreach ($customization['datas'][1] as $key => $customization_datas) {
                                    // message
                                    if ($customization_datas['id_customization_field'] == 1) {
                                        $message = $customization_datas['value'];
                                    }
                                    //firstname
                                    if ($customization_datas['id_customization_field'] == 3) {
                                        $firstname = $customization_datas['value'];
                                    }
                                    // leastname
                                    if ($customization_datas['id_customization_field'] == 4) {
                                        $lastname = $customization_datas['value'];
                                    }
                                    // email
                                    if ($customization_datas['id_customization_field'] == 2) {
                                        if (!empty($customization_datas['value'])) {
                                            if (!Validate::isEmail($customization_datas['value'])) {
                                                $template1 = 'error_card_sent';
                                                $title1 =  $this->lgc('Error when card send !', $customer->id_lang);
                                                $message_error = '<span style="font-weight:bold;">'.
                                                $this->lgc('You have try to send the card to:', $customer->id_lang).
                                                ' '.
                                                $customization_datas['value'].
                                                '</span><br /><span style="font-weight:bold;">'.
                                                $this->lgc('but this email is not valid', $customer->id_lang).
                                                '</span><br /><span style="font-weight:bold;">'.
                                                $this->lgc(
                                                    'You find this card in another email for transfer to your friend.',
                                                    $customer->id_lang
                                                ).
                                                '</span>';
                                                Mail::Send(
                                                    ...
                                                    $template1,
                                                    $title1,
                                                    array('{message_error}' => $message_error,),
                                                    $customer->email,
                                                    null,
                                                    null,
                                                    null,
                                                ...
                                                $email = $customization_datas['value'];
                                            }
                                        }
                                    }
                                }
                                ...
                                if ($email != $customer->email) {
                                    ...
                                    $message_confirm =
                                        $this->lgc('Your Gift Card has been sent to:', $customer->id_lang)
                                        .' '.
                                        $email;
                                    Mail::Send(
                                        (int)$customer->id_lang,
                                        $template1,
                                        $title1,
                                        array('{message_confirm}' => $message_confirm,),
                                        $customer->email,
                                        null,
                                        null,
                                        null,
                                ...
                                $order->cardimage = dirname(__FILE__).
                                    '/cards/Order-'.
                                    $order->id.
                                    '-Product-'.
                                    $product['product_id'].
                                ...
                                $voucher_price = ($giftcard->tax == 1) ?
                                    $card_product->price *((100 + $this->getTaxRate())/100) :
                                    $card_product->price;
                                $voucher_price1 = preg_replace('/,/', '.', $voucher_price);
                                $cart_rule->reduction_tax = $giftcard->tax;
                                $cart_rule->minimum_amount_currency = 1;
                                $cart_rule->reduction_currency = $currency->id;;
                                $cart_rule->reduction_amount = Tools::convertPriceFull(
                                    $voucher_price1,
                                    null,
                                    $currency
                                );
                                $cart_rule->free_shipping = $giftcard->free_shipping;
                                $cart_rule->add();
                ...
                if ($classic > 0) {
                    for ($i = 0; $i < $classic; $i++) {
                        ...
                        $email = $customer->email;
                        $products_data = '';
                        $products_data .='<p></p><br /><br /><p><span style="font-weight:bold;">'.
                            $giftcard_data.
                            '</span><a href="'.
                            $productLink.
                            '">'.
                            $product['product_name'].
                            '</a></p><br /><br />';
                        $my_vouchers = Context::getContext()->link->getPageLink(
                            'discount',
                            true,
                            $customer->id_lang,
                            null,
                        ...
                        $order->cardimage = dirname(__FILE__).
                            ...
                            $product['product_id'].
                            '.png';
                        $order->title = $this->lgc('Your Gift Card', $customer->id_lang);
                        $order->pdftextcontent = $pdftextcontent;
                        $order->pdftextfooter = $pdftextfooter;
                        $order->conditions = $GiftCardConditions;
                        $pdf = new PDF($order, 'CardPdf', Context::getContext()->smarty);
                        ...
                        $cart_rule->minimum_amount_currency = 1;
                        $cart_rule->reduction_currency = $currency->id;
                        $cart_rule->reduction_amount = Tools::convertPriceFull($voucher_price1, null, $currency);
                        $cart_rule->free_shipping = $giftcard->free_shipping;
                        $cart_rule->highlight = $giftcard->highlight;
                        $cart_rule->add();
                        StockAvailable::updateQuantity($product['product_id'], 0, 1, $id_shop);
                    }
                }
    ...
    public static function defaultTrans()
    {
        ...
        $trans = array(
            ...
            $this->l('Date of mailing of the card by Email'),
            $this->l('The amount value will be use only in'),
            $this->l('Month'),
            $this->l('WT'),
            $this->l('ATI'),
            ...
            $this->l('Error when card send !'),
            $this->l('You have try to send the card to:'),
            $this->l('but this email is not valid'),
            $this->l('You find this card in another email for transfer to your friend.'),
            $this->l('Add a message on the card'),
            $this->l('If you want send this card to a friend enter his Email address (If the email address is bad you will receive the card on your email).'),
            $this->l('Friend first name (display in the mail)'),
            $this->l('Friend last name (display in the mail)'),
        );
    }
